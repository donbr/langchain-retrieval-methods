...
project_name = f"ragas-golden-dataset-pipeline-{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
@task(
    ...
    description="Loads documents from a Postgres table using SQLDatabaseLoader",
    retries=2,
    retry_delay_seconds=10,
    tags=["data", "loading", "postgres"]
)
def load_documents_from_postgres() -> List[Document]:
    logger = get_run_logger()
    try:
        # Use SUPABASE_DB_URL for connection
        db_url = os.environ["SUPABASE_DB_URL"]
        # Use the docloader table and schema
        table = "azure_arch_cert_documents"
        content_column = "content"
        metadata_column = "metadata"
        # Build SQLAlchemy connection string for SQLDatabaseLoader
        # Convert psycopg2 URL to SQLAlchemy format if needed
        if db_url.startswith("postgres://"):
            conn_str = db_url.replace("postgres://", "postgresql+psycopg2://", 1)
        elif db_url.startswith("postgresql://"):
            conn_str = db_url.replace("postgresql://", "postgresql+psycopg2://", 1)
        else:
            conn_str = db_url
        query = f"SELECT {content_column} as page_content, {metadata_column} as metadata FROM {table}"
        from langchain_community.utilities.sql_database import SQLDatabase
        from langchain_community.document_loaders.sql_database import SQLDatabaseLoader
        ...
        logger.info(f"Loaded {len(docs)} documents from Postgres table '{table}'")
        return docs
    except Exception as e:
        logger.error(f"Failed to load documents from Postgres: {e}")
        create_markdown_artifact(
            key="postgres-loading-error",
            markdown=f"# ❌ Postgres Loading Failed\n\n**Error:**\n```\n{str(e)}\n```",
            description="Postgres loading error"
        )
        raise
...
def ragas_pipeline(
    ...
    try:
        ...
        # 3. Load documents from Postgres (replace PDF download/loading)
        docs_future = load_documents_from_postgres.submit()
        docs = docs_future.result()
        if not docs:
            logger.error("No documents loaded from Postgres — aborting pipeline.")
            create_markdown_artifact(
                key="pipeline-error",
                markdown="# ❌ Pipeline Aborted\n\nNo documents were loaded from Postgres. Cannot proceed with testset generation.",
                description="Pipeline error"
            )
            return {"status": "failed", "reason": "No documents loaded from Postgres"}

        # 4. Submit document saving task in parallel with other tasks
        save_future = save_documents.submit(docs, PROCESSED_DIR)

        # 5. Submit testset generation task
        dataset_future = build_testset.submit(
            docs, TESTSET_SIZE, KG_OUTPUT_PATH, LLM_MODEL, EMBEDDING_MODEL
        )

        # 6. Get saved paths and testset results
        ...
        # 8. Create final success artifact
        create_markdown_artifact(
